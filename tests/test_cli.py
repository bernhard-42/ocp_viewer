"""Tests for ocp_viewer's CLI — verify that command-line flags propagate
into the running viewer's workspace config.

Each test spawns its own `python -m ocp_viewer` subprocess on a non-default
...
from ocp_viewer import set_port, workspace_config
...
def disable_pytest_stub():
    ...
    One variable, where ocp_vscode's copy of this had to pop two: the stub is
    ocp-viewer-core's, and this package has no second one of its own.
    """
    old = os.environ.pop("OCP_VIEWER_PYTEST", None)
    yield
    if old is not None:
        os.environ["OCP_VIEWER_PYTEST"] = old
...
@pytest.fixture
def standalone():
    """Spawn `python -m ocp_viewer` with caller-supplied CLI args.
...
def test_collapse_accepts_string_value(standalone):
    ...
    from ocp_viewer import Collapse
...
def test_status_after_show_returns_safe_collapse(standalone):
    ...
    import cadquery as cq
    from ocp_viewer import Collapse, show, status
...
def test_send_fallback_does_not_poison_collapse():
    ...
    result = comms._send("config", MessageType.COMMAND, DEAD_PORT)
...
# A test was dropped here rather than carried across: it compared this package's
# DEFAULTS against the VS Code extension's package.json settings, and could only
# run while one repository held both. 32 keys are defaulted in both packages and
# they agree today, so the drift it guarded is real and is now unguarded - the
# fix is for the core to own those defaults as it owns the renderer's, not for a
# test here to reach into a sibling repository.
